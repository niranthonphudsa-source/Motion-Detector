...
class PoseTkinterGUI:
    def __init__(self, window):
        ...
        self.prev_time = 0
        self.frame_count = 0  # ตัวนับลำดับเฟรมสำหรับบันทึกลง CSV
        ...
        # 📝 สร้างไฟล์ CSV ใหม่และเขียนหัวตาราง (Header) ตั้งแต่เริ่มโปรแกรม
        with open(filename, mode='w', newline='', encoding='utf-8-sig') as file:
            writer = csv.writer(file)
            writer.writerow(["Timestamp", "Frame_ID", "Point_ID", "X", "Y"])
    ...
    def update_frame(self):
        ...
        if ret and frame is not None:
            ...
            # เปิดไฟล์ CSV แบบเขียนต่อท้าย ('a') เพื่อบันทึกพิกัดของเฟรมนี้
                ...
                for result in results:
                    if result.keypoints is not None:
                        ...
                        for keypoints in keypoints_list:
                            if len(keypoints) < 17: continue  # ตรวจสอบจุดให้ครบตามมาตรฐาน COCO 17 จุด
                            
                            # ดึงพิกัดที่ใช้คำนวณท่าทาง
                            ...
                            if left_shoulder[0] > 0 and right_shoulder[0] > 0 and right_wrist[0] > 0 and left_wrist[0] > 0:
                                ...
                                # แก้ไขสูตรระยะฝั่งขวาตรงนี้เล็กน้อยให้เป็นการลบเครื่องหมายเวกเตอร์ที่ถูกต้อง
                                distance_left_wrist_to_shoulder = np.linalg.norm(left_wrist - right_shoulder) 
                            ...
                            # 🦴 1. วาดเส้นโครงกระดูก และ 💾 2. บันทึกพิกัดทุกจุดที่ตรวจเจอลง CSV
                            ...
                            for connection in self.SKELETON_CONNECTIONS:
                                ...
                                if x1 > 0 and y1 > 0 and x2 > 0 and y2 > 0:
                                    ...
                                    cv2.putText(frame, f"{pt1_idx}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255,0,255), 1)
                                    cv2.putText(frame, f"{pt2_idx}", (x2, y2), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255,0,255), 1)

                            # วนลูปบันทึกพิกัด (X, Y) ทั้ง 17 จุดลง CSV (บันทึกเฉพาะจุดที่โมเดลหาเจอค่า > 0)
                            for idx, kp in enumerate(keypoints):
                                kpx, kpy = int(kp[0]), int(kp[1])
                                if kpx > 0 and kpy > 0:
                                    writer.writerow([timestamp_str, self.frame_count, idx, kpx, kpy])
                                    
                                    # วาดจุดวงกลมข้อต่อ
                                    cv2.circle(frame, (kpx, kpy), 5, (255, 255, 255), cv2.FILLED)
                                    cv2.circle(frame, (kpx, kpy), 5, theme_color, 1)
